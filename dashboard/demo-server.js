...
const DATA = {
  ...
  '/api/backups': () => [
    ...
    { Status:'WARN', ServerName:'SQLPROD03', DatabaseName:'StagingDB', StateDesc:'ONLINE', RecoveryModel:'SIMPLE', LastFullBackup:new Date(Date.now()-3*864e5).toISOString().slice(0,19), HoursSinceFull:72, LastLogBackup:null, LastGoodCheckDb:null, PageVerify:'TORN_PAGE_DETECTION', IsAutoShrink:true },
    { Status:'OK', ServerName:'SQLPROD01\\AG', DatabaseName:'SalesDB', StateDesc:'ONLINE', RecoveryModel:'FULL', LastFullBackup:new Date(Date.now()-6*36e5).toISOString().slice(0,19), HoursSinceFull:6, LastLogBackup:new Date(Date.now()-12*6e4).toISOString().slice(0,19), LastGoodCheckDb:new Date(Date.now()-3*864e5).toISOString().slice(0,19), PageVerify:'CHECKSUM', IsAutoShrink:false },
  ],
  '/api/jobs': () => [
  ...
  '/api/vitals': () => [
    { Status:'CRIT', Platform:'MSSQL', ServerName:'SQLPROD03', MetricName:'suspect_pages', MetricValue:2 },
    { Status:'CRIT', Platform:'MSSQL', ServerName:'SQLPROD03', MetricName:'blocked_sessions', MetricValue:3 },
    { Status:'WARN', Platform:'MSSQL', ServerName:'SQLPROD03', MetricName:'cpu_pct', MetricValue:91 },
    { Status:'OK', Platform:'MSSQL', ServerName:'SQLPROD03', MetricName:'deadlocks_total', MetricValue:147 },
