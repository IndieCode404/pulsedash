...
## This is the Phase 1 POC

This folder contains **only** the approved POC scope — the code that ships to the
client machine. Advanced features (advisor, bottlenecks, cost anomaly, config audit,
access control, index health) exist in a separate Phase 2 archive and will merge
back after POC approval. Do not build Phase 2 features here.

The architecture is designed to extend to **Oracle, Aurora PostgreSQL, Db2, and
Couchbase** — see `docs/EVOLUTION.md`. Keep multi-engine extensibility in mind
when modifying schema or the dashboard, but don't build other engine collectors now.
...
- This is Phase 1 (POC scope only). Phase 2 features exist separately.
...
- KIRO-AUTOMODEL.md — master instruction file (conventions, pitfalls, recipes)
- docs/EVOLUTION.md — multi-engine roadmap (Oracle, Aurora, Db2, Couchbase)
